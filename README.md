...
Note that the AMI referenced in those slides (`ami-0ea5e9630dcf98849`) ships
  PyTorch 2.0.1, while this assignment pins `torch==2.2.2` in
  `requirements.txt`. Create the conda environment and install the requirements
  as described below instead of relying on the PyTorch that comes with the AMI.
